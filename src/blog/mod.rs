...
impl BlogManager {
    ...
    pub async fn create_post(&mut self, post: BlogPost) -> Result<String> {
        ...
        if let Some(existing_storage_id) = self.database.get_post_by_content_hash(&post.content_hash).await? {
    ...
    pub async fn publish_post(&mut self, storage_id: &str) -> Result<()> {
        self.database.update_post_published(storage_id, true).await?;
        Ok(())
    }
